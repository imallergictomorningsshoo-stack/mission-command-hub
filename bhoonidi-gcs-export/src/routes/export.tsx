...
const options = [
  {
    ...
    title: "Export CSV",
    detail: "Full packet log with all telemetry fields · ~18 KB",
    action: "Download CSV",
    ready: true,
  },
  {
    icon: FileJson,
    title: "Export JSON",
    detail: "Structured dataset including mission metadata",
    action: "Coming Soon",
    ready: false,
  },
  {
    ...
    detail: "Formatted mission report with charts and summary",
    action: "Coming Soon",
    ready: false,
  },
];
  ...
  return (
    <main className="mx-auto max-w-[1120px] px-6 py-10">
            ...
            <div className="flex items-center gap-4">
              <span
                className={`grid size-11 place-items-center rounded-xl border ${
                  ready ? "border-signal/30 bg-signal/10" : "border-border bg-panel/50"
                }`}
              >
                <Icon
                  className={`size-5 ${ready ? "text-signal" : "text-muted-foreground"}`}
                  strokeWidth={1.7}
                />
              </span>
              <div>
      ...
      <p className="mt-6 text-center text-xs text-muted-foreground">
        Exports contain raw downlinked telemetry with no post-processing applied.
      </p>
    </main>
  );
}
