    ...
    <main className="mx-auto max-w-[1600px] px-6 py-8">
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          <TelemetryCard label="Altitude" value={latest.altitude.toFixed(1)} unit="m AGL" icon={Mountain} delta={`${Math.abs(latest.altitude - prev.altitude).toFixed(1)} m`} trend={latest.altitude >= prev.altitude ? "up" : "down"} />
          <TelemetryCard label="Pressure" value={latest.pressure.toFixed(2)} unit="hPa" icon={Gauge} accent="accent" delta={`${Math.abs(latest.pressure - prev.pressure).toFixed(2)} hPa`} trend={latest.pressure >= prev.pressure ? "up" : "down"} />
          <TelemetryCard label="Temperature" value={latest.temperature.toFixed(2)} unit="°C" icon={Thermometer} accent="warn" delta={`${Math.abs(latest.temperature - prev.temperature).toFixed(2)} °C`} trend={latest.temperature >= prev.temperature ? "up" : "down"} />
          <TelemetryCard label="Tilt" value={latest.tilt.toFixed(1)} unit="deg" icon={Compass} accent="ok" delta={`${Math.abs(latest.tilt - prev.tilt).toFixed(1)}°`} trend={latest.tilt >= prev.tilt ? "up" : "down"} />
        </div>
      ...
      <Panel className="mt-5">
        <PanelHeader title="Telemetry Log" hint="Live Stream" right={<StatusChip tone="online" pulse>Receiving</StatusChip>} />
        <TelemetryTable rows={recent as any} />
      </Panel>
    </main>
  );
}
